...
export function readFile(filepath: string, startLine = 1, endLine = 300): string {
  try {
    ...
    console.log(`[Tool] read_file: ${relative} (${startLine}-${endLine})`);
...
export function searchProject(keyword: string, maxResults = 30): string {
  try {
    ...
    console.log(`[Tool] search_project: "${keyword}" (limit=${safeLimit})`);
...
export const agentTools: any[] = [
  {
    ...
    function: {
      name: 'read_file',
      description:
        'Read a project file before editing it. Use relative paths from repo root. Supports line range for targeted inspection.',
      parameters: {
        type: 'object',
        properties: {
          filepath: {
            type: 'string',
            description: 'Relative path like "kubo-mobile/app/(tabs)/explore.tsx".',
          },
          startLine: {
            type: 'number',
            description: 'Optional first line number (1-based).',
          },
          endLine: {
            type: 'number',
            description: 'Optional last line number (1-based).',
          },
        },
        required: ['filepath'],
      },
  ...
  },
  {
    ...
    function: {
      name: 'search_project',
      description:
        'Search keyword usage across source files and return matching file paths with line snippets.',
      parameters: {
        type: 'object',
        properties: {
          keyword: {
            type: 'string',
            description: 'String token like "KuboFilterModal" or "JwtAuthGuard".',
          },
          maxResults: {
            type: 'number',
            description: 'Optional cap for matching files (default 30, max 100).',
          },
        },
        required: ['keyword'],
      },
